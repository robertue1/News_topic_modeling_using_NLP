...
rows = engine.execute(f"SELECT title, url, date FROM news_table WHERE title LIKE COLLATE Latin1_general_CI_AI '%{word}%' COLLATE Latin1_General_CI_AI \
AND date LIKE '%{year}%' LIMIT {num_art}").fetchall()
